...
where
  // 这里的PartialOrd限制是为了执行大小比对。Serialize参数可以放到Err里
...
pub fn validate_collection_name(
  ...
  match INVALID_CHARS.into_iter().find(|c| value.contains(*c)) {
    Some(c) => {
      ...
      err.message.replace(
        format!("collection name cannot contain \"{c}\" char").into(),
      );
      Err(err)
    }
    None => Ok(()),
...
mod tests {
  ...
  fn test_validate_range_generic() {
    ...
    assert!(
      validate_range_generic(0.0, Some(1.0), Some(1.0)).is_err()
    );
    assert!(validate_range_generic(1.0, Some(1.0), Some(1.0)).is_ok());
    assert!(
      validate_range_generic(2.0, Some(1.0), Some(1.0)).is_err()
    );
    assert!(
      validate_range_generic(0.0, Some(1.0), Some(2.0)).is_err()
    );
    assert!(validate_range_generic(1.0, Some(1.0), Some(2.0)).is_ok());
    assert!(validate_range_generic(2.0, Some(1.0), Some(2.0)).is_ok());
    assert!(
      validate_range_generic(3.0, Some(1.0), Some(2.0)).is_err()
    );
    assert!(
      validate_range_generic(0.0, Some(2.0), Some(1.0)).is_err()
    );
    assert!(
      validate_range_generic(1.0, Some(2.0), Some(1.0)).is_err()
    );
    assert!(
      validate_range_generic(2.0, Some(2.0), Some(1.0)).is_err()
    );
    assert!(
      validate_range_generic(3.0, Some(2.0), Some(1.0)).is_err()
    );
  }
