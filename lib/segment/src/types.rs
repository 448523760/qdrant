...
use crate::data_types::vectors::{
  VectorElementType, VectorStruct, VectorType,
};
use crate::spaces::metric::Metric;
use crate::spaces::simple::{
  CosineMetric, DotProductMetric, EuclidMetric,
};
...
#[derive(
  Debug, Deserialize, Serialize, JsonSchema, Clone, PartialEq, Eq,
)]
#[serde(rename_all = "snake_case")]
...
#[derive(
  Default, Debug, Deserialize, Serialize, JsonSchema, Clone,
)]
#[serde(rename_all = "snake_case")]
...
impl SegmentConfig {
  ...
  pub fn is_any_vector_indexed(&self) -> bool {
    self.vector_data.values().any(|config| config.index.is_indexed())
  }

  pub fn are_all_vectors_indexed(&self) -> bool {
    self.vector_data.values().all(|config| config.index.is_indexed())
  }
...
impl GeoPoint {
  pub fn validate(
    lon: f64,
    lat: f64,
  ) -> Result<(), GeoPointValidationError> {
    let max_lon = 180f64;
    let min_lon = -180f64;
    let max_lat = 90f64;
    let min_lat = -90f64;
  ...
  pub fn new(
    lon: f64,
    lat: f64,
  ) -> Result<Self, GeoPointValidationError> {
...
impl TryFrom<GeoPointShadow> for GeoPoint {
  ...
  fn try_from(value: GeoPointShadow) -> Result<Self, Self::Error> {
    ...
    Ok(Self { lon: value.lon, lat: value.lat })
  }
}
...
#[derive(
  Clone, Debug, Eq, PartialEq, Deserialize, Serialize, JsonSchema,
)]
...
/// ```
#[derive(
  Debug, Deserialize, Serialize, JsonSchema, PartialEq, Eq, Clone,
)]
#[serde(rename_all = "snake_case")]
#[serde(untagged)]
...
impl TryFrom<PayloadIndexInfo> for PayloadFieldSchema {
  ...
  fn try_from(
    index_info: PayloadIndexInfo,
  ) -> Result<Self, Self::Error> {
    match (index_info.data_type, index_info.params) {
      (
        PayloadSchemaType::Text,
        Some(PayloadSchemaParams::Text(params)),
      ) => Ok(PayloadFieldSchema::FieldParams(
      ...
      (data_type, None) => {
        Ok(PayloadFieldSchema::FieldType(data_type))
      }
    }
  }
}
...
pub fn infer_collection_value_type<'a, I>(
  values: I,
...
{
  let possible_types =
    values.into_iter().map(value_type).unique().collect_vec();
...
impl From<MatchInterface> for Match {
  fn from(value: MatchInterface) -> Self {
    match value {
      MatchInterface::Value(value) => {
        Self::Value(MatchValue { value: value.value })
      }
      MatchInterface::Text(text) => {
        Self::Text(MatchText { text: text.text })
      }
      MatchInterface::Any(any) => {
        Self::Any(MatchAny { any: any.any })
      }
      MatchInterface::Except(except) => {
        Self::Except(MatchExcept { except: except.except })
      }
    }
  }
}
...
impl From<Vec<String>> for Match {
  fn from(keywords: Vec<String>) -> Self {
    Self::Any(MatchAny { any: AnyVariants::Keywords(keywords) })
  }
}

impl From<Vec<String>> for MatchExcept {
  fn from(keywords: Vec<String>) -> Self {
    MatchExcept { except: AnyVariants::Keywords(keywords) }
  }
}

impl From<Vec<IntPayloadType>> for Match {
  fn from(integers: Vec<IntPayloadType>) -> Self {
    Self::Any(MatchAny { any: AnyVariants::Integers(integers) })
  }
}

impl From<Vec<IntPayloadType>> for MatchExcept {
  fn from(integers: Vec<IntPayloadType>) -> Self {
    MatchExcept { except: AnyVariants::Integers(integers) }
  }
}

/// Range filter request
#[derive(
  Debug, Deserialize, Serialize, JsonSchema, Default, Clone, PartialEq,
)]
#[serde(rename_all = "snake_case")]
pub struct Range {
  /// point.key < range.lt
...
impl GeoRadius {
  pub fn check_point(&self, lon: f64, lat: f64) -> bool {
    ...
    query_center.haversine_distance(&Point::new(lon, lat))
      < self.radius
  }
}
...
impl From<String> for IsNullCondition {
  fn from(key: String) -> Self {
    IsNullCondition { is_null: PayloadField { key } }
  }
}

impl From<String> for IsEmptyCondition {
  fn from(key: String) -> Self {
    IsEmptyCondition { is_empty: PayloadField { key } }
  }
}
...
impl Condition {
  pub fn new_nested(key: impl Into<String>, filter: Filter) -> Self {
    Self::Nested(NestedCondition {
      nested: Nested { key: key.into(), filter },
    })
  }
}
...
impl From<bool> for WithPayload {
  fn from(x: bool) -> Self {
    WithPayload { enable: x, payload_selector: None }
  }
}
...
impl From<&WithPayloadInterface> for WithPayload {
  fn from(interface: &WithPayloadInterface) -> Self {
    match interface {
      WithPayloadInterface::Bool(x) => {
        WithPayload { enable: *x, payload_selector: None }
      }
      WithPayloadInterface::Fields(x) => WithPayload {
        enable: true,
        payload_selector: Some(PayloadSelector::new_include(
          x.clone(),
        )),
      },
...
impl PayloadSelector {
  pub fn new_include(
    vecs_payload_key_type: Vec<PayloadKeyType>,
  ) -> Self {
...
#[derive(
  Debug, Deserialize, Serialize, JsonSchema, Clone, PartialEq, Default,
)]
#[serde(deny_unknown_fields)]
#[serde(rename_all = "snake_case")]
pub struct Filter {
...
impl Filter {
  ...
  pub fn merge(&self, other: &Filter) -> Filter {
    ...
    Filter {
      should: merge_component(
        self.should.clone(),
        other.should.clone(),
      ),
      must: merge_component(self.must.clone(), other.must.clone()),
      must_not: merge_component(
        self.must_not.clone(),
        other.must_not.clone(),
      ),
    }
  }
}
...
mod tests {
  ...
  #[allow(dead_code)]
  fn check_rms_serialization<
    T: Serialize + DeserializeOwned + PartialEq + std::fmt::Debug,
  >(
    record: T,
  ) {
    let binary_entity =
      rmp_serde::to_vec(&record).expect("serialization ok");
    let de_record: T = rmp_serde::from_slice(&binary_entity)
  ...
  fn test_rmp_vs_cbor_deserialize() {
    let payload: Payload =
      json!({"payload_key":"payload_value"}).into();
  ...
  fn test_geo_boundingbox_check_point() {
    let bounding_box = GeoBoundingBox {
      top_left: GeoPoint { lon: -1.0, lat: 1.0 },
      bottom_right: GeoPoint { lon: 1.0, lat: -1.0 },
    };
  ...
  fn test_parse_match_query() {
    ...
    assert_eq!(
      ...
      Match::Value(MatchValue { value: ValueVariants::Integer(42) })
    );
    ...
    assert_eq!(
      condition.r#match.unwrap(),
      Match::Value(MatchValue { value: ValueVariants::Bool(true) })
    );
  ...
  fn test_parse_nested_filter_query() {
    ...
    match musts.get(0) {
      Some(Condition::Nested(nested_condition)) => {
        ...
        let nested_musts =
          nested_condition.filter().must.as_ref().unwrap();
  ...
  fn test_remove_key() {
    ...
    .unwrap();
    let removed =
      remove_value_from_json_map("b.c", &mut payload.0).values();
    assert_eq!(removed, vec![Value::Number(123.into())]);
    assert_ne!(payload, Default::default());

    let removed =
      remove_value_from_json_map("b.e.f[1]", &mut payload.0).values();
    assert_eq!(removed, vec![Value::Number(2.into())]);
    assert_ne!(payload, Default::default());

    let removed =
      remove_value_from_json_map("b.e.i[0].j", &mut payload.0)
        .values();
    assert_eq!(removed, vec![Value::Number(1.into())]);
    assert_ne!(payload, Default::default());

    let removed =
      remove_value_from_json_map("b.e.i[].k", &mut payload.0)
        .values();
    assert_eq!(
      removed,
    ...
    let removed =
      remove_value_from_json_map("b.e.i[]", &mut payload.0).values();
    assert_eq!(
      removed,
      vec![Value::Array(vec![
    ...
    let removed =
      remove_value_from_json_map("b.e.f", &mut payload.0).values();
    ...
    let removed =
      remove_value_from_json_map("a", &mut payload.0).values();
    assert_eq!(removed, vec![Value::Number(1.into())]);
    assert_ne!(payload, Default::default());

    let removed =
      remove_value_from_json_map("b.e", &mut payload.0).values();
    assert_eq!(
      removed,
    ...
    let removed =
      remove_value_from_json_map("b", &mut payload.0).values();
    assert_eq!(
      removed,
      vec![Value::Object(serde_json::Map::from_iter(vec![]))]
    ); // empty object left
  ...
  fn test_payload_parsing() {
    ...
    let ft = PayloadFieldSchema::FieldParams(
      PayloadSchemaParams::Text(Default::default()),
    );
    let ft_json = serde_json::to_string(&ft).unwrap();
    eprintln!("ft_json = {ft_json:?}");

    let query = r#""keyword""#;
    let field_type: PayloadSchemaType =
      serde_json::from_str(query).unwrap();
