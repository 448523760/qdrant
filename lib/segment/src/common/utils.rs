...
impl<T> MultiValue<T> {
  ...
  pub(crate) fn as_ref(&self) -> MultiValue<&T> {
    match self {
      ...
      Self::Multiple(vec) => {
        MultiValue::Multiple(vec.iter().collect())
      }
    }
  }
}
...
fn parse_array_path(path: &str) -> Option<(&str, Option<u32>)> {
  ...
  let index = path.next();
  debug!("opt_element: {:?}, opt_index: {:?}", element, index);
  match (element, index) {
            ...
            match rest_path {
              Some(rest_path) => {
                ...
                if let Value::Object(map) = value {
                  values
                    .extend(get_value_from_json_map(rest_path, map))
                }
              }
  ...
  match path.split_once('.') {
    Some((element, rest_path)) => {
      ...
      match parse_array_path(element) {
        Some((array_element_path, array_index)) => focus_array_path(
          array_element_path,
          array_index,
          Some(rest_path),
          value,
        ...
        None => {
          ...
          match value.get(element) {
            Some(Value::Object(map)) => {
              get_value_from_json_map(rest_path, map)
            }
      ...
      None => {
        ...
        if let Some(array_index) = array_index {
          if array.len() > array_index as usize {
            return MultiValue::one(
              array.remove(array_index as usize),
            );
          }
        } else {
          return MultiValue::one(Value::Array(std::mem::take(
            array,
          )));
        }
      }
        ...
        for (i, value) in array.iter_mut().enumerate() {
          if let Value::Object(map) = value {
            if let Some(array_index) = array_index {
              if i == array_index as usize {
                values
                  .extend(remove_value_from_json_map(rest_path, map));
              }
            } else {
              values
                .extend(remove_value_from_json_map(rest_path, map));
...
pub fn remove_value_from_json_map(
  ...
  match path.split_once('.') {
    Some((element, rest_path)) => {
      // check if targeting array
      match parse_array_path(element) {
        Some((array_element_path, array_index)) => delete_array_path(
          array_element_path,
          array_index,
          Some(rest_path),
          value,
        ...
        None => {
          ...
          if rest_path.is_empty() {
            ...
            match value.get_mut(element) {
              None => MultiValue::default(),
              Some(Value::Object(map)) => {
                remove_value_from_json_map(rest_path, map)
              }
